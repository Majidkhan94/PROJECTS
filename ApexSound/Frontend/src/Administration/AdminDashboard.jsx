    ...
    <section className="h-screen w-full flex overflow-hidden">
      ...
      <div className="h-full w-70 bg-black flex flex-col justify-between">
        ...
        <div className="flex items-center justify-center py-6 border-b border-gray-800">
          <img src="/logo.png" alt="ApexSound" className="h-9 w-auto object-contain" />
        </div>
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-t border-gray-800">
          ...
          />
          <div>
            <p className="text-sm font-medium text-white">Majid Khan</p>
      ...
      <div className="flex-1 relative overflow-hidden">
        ...
        {active === "dashboard" && (
          <Section key="dashboard" className="bg-amber-600">
            Dashboard
          </Section>
        )}

        {active === "users" && (
          <Section key="users" className="bg-blue-800">
            User Management
          </Section>
        )}

        {active === "vendors" && (
          <Section key="vendors" className="bg-green-400">
            venderrequest
          </Section>
        )}

        {active === "categories" && (
          <Section key="categories" className="bg-purple-600">
            categories
          </Section>
        )}

        {active === "products" && (
          <Section key="products" className="bg-emerald-700">
            ProductManagement
          </Section>
        )}

        {active === "orders" && (
          <Section key="orders" className="bg-amber-950">
            ordermanagement
          </Section>
        )}
