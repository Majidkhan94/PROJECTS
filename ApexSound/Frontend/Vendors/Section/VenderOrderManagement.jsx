...
import { GetOrdersByVendor, UpdateOrderStatus } from "../../APIs/OrderAPIs.js";

export const VenderOrderManagement = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await GetOrdersByVendor(userId);
      if (response.success) {
    ...
    <section className="mx-5 mt-5">
      ...
      <div className="mx-5 mt-5">
        ...
        <div className="flex flex-wrap gap-5 mt-6">
          {filteredOrders.map((order) => (
            <div key={order.id} className="bg-background-color rounded-2xl p-6 w-[calc(33.333%-1.34rem)]">
              ...
              <div className="mt-6">
                <select
                  className="w-full rounded-full px-3 py-2 bg-white/5 text-hover-bg"
                  value={order.status}
        ...
        {filteredOrders.length === 0 && (
          <PageHeader text={"No orders found."} className={"text-button-redhover"} />
        )}
      </div>
